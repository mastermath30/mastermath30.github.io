      ...
      <main className="max-w-7xl mx-auto px-6 py-8 pb-32">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card padding="none" className="overflow-hidden">
              ...
              <div>
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                  <h3 className="font-semibold text-sm text-slate-500 flex items-center gap-2">
                ...
                <div className="divide-y divide-slate-100">
                  {sortedPosts.length === 0 ? (
                    <div className="p-8 text-center">
                      ...
                      <p className="text-slate-500">No posts yet. Be the first to ask a question!</p>
                    </div>
                  ) : (
                    sortedPosts.map((post) => (
                      <div key={post.id} className="p-5 hover:bg-slate-50 transition-colors">
                        <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-xl bg-violet-100 flex items-center justify-center text-violet-500 shrink-0">
          ...
          <div className="space-y-6">
              ...
              <div className="divide-y divide-slate-100">
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-900 font-mono">1,245</p>
                  </div>
                </div>
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-900 font-mono">{328 + posts.length}</p>
                  </div>
                </div>
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-900 font-mono">5,723</p>
                  </div>
                </div>
              </div>
            </Card>
            ...
            {/* Top Contributors */}
            <Card padding="none" className="overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-gradient-to-r from-amber-50 to-violet-50">
                <h2 className="font-semibold text-slate-900 flex items-center gap-2">
                ...
                {topContributors.map((contributor, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-sm text-slate-900">{contributor.name}</p>
            ...
            <Card>
              ...
              <div className="space-y-3">
                <Link href="/resources" className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-violet-50 text-slate-600 hover:text-violet-600 transition-all text-sm">
                  <BookOpen className="w-5 h-5" />
                  Browse Resources
                </Link>
                <Link href="/schedule" className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-violet-50 text-slate-600 hover:text-violet-600 transition-all text-sm">
                  <Calendar className="w-5 h-5" />
                  Book a Session
                </Link>
                <Link href="/support" className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-violet-50 text-slate-600 hover:text-violet-600 transition-all text-sm">
                  <HelpCircle className="w-5 h-5" />
                  Get Help
                </Link>
              </div>
