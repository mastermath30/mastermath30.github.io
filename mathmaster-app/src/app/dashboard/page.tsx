    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-violet-600/95 via-violet-500/90 to-purple-500/95" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-12">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="text-white">
              ...
              <p className="text-violet-100 text-lg">Track your progress and stay on top of your learning journey.</p>
              ...
              <div className="flex gap-6 mt-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-violet-200">Mastery</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 pb-32">
          ...
          <Card padding="none" className="overflow-hidden">
            ...
            <div className="divide-y divide-slate-100">
              {challenges.map((challenge) => (
                <div key={challenge.title} className="p-4 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {!challenge.isQuiz && (
                        <div className="mt-2">
                          ...
                          <p className="text-slate-500 text-xs mt-1">{challenge.completed}/{challenge.total} completed</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100">
              <Link href="#" className="text-violet-500 text-sm font-medium flex items-center gap-2 hover:gap-3 transition-all">
                View all challenges
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          {/* Recent Activity */}
          <Card className="overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 -mx-6 -mt-6 px-6 pt-6 pb-4 mb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
              ...
              <div className="space-y-4">
                {activities.map((activity, index) => (
                  <div key={index} className="flex gap-4 pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                    ...
                    <div>
                      <h4 className="font-medium text-sm text-slate-900">{activity.title}</h4>
                      <p className="text-slate-500 text-xs mt-0.5">{activity.time}</p>
                      {activity.xp && (
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-violet-500 text-xs font-medium bg-violet-50 px-2 py-0.5 rounded-full">{activity.xp}</span>
                          <span className="text-slate-500 text-xs">{activity.badge}</span>
                        </div>
                      )}
                      {activity.preview && (
                      ...
                      {activity.details && (
                        <p className="text-slate-500 text-xs mt-2">{activity.details}</p>
                      )}
                    </div>
                  </div>
                ))}
              </div>
              <Link href="#" className="text-violet-500 text-sm font-medium flex items-center gap-2 hover:gap-3 transition-all mt-4">
                View all activity
                <ArrowRight className="w-4 h-4" />
              </Link>
            </CardContent>
              ...
              {goals.map((goal) => (
                <div key={goal.title} className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-500 text-xs font-mono">{goal.label}</span>
                  </div>
                </div>
              ))}
            </div>
